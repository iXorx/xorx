...
3. Run `pnpm run export:static` to generate the plain HTML/JS/CSS bundle in `out/`. Note: newer Next.js releases removed the `next export` CLI. This template sets `NEXT_OUTPUT_EXPORT=1` during the build to enable the `output: 'export'` static export mode in `next.config.js` when exporting. The build process copies `public/.nojekyll` into `out/` so `_next` assets are not ignored by GitHub Pages.
